...
export class BrokerImport {
  ...
  handleFileUpload(brokerId, file, card, fileInput, statusDiv) {
    ...
    this.showUploadSuccess(card);

    if (statusDiv) {
      statusDiv.innerHTML =
        '<span class="status-success">✅ Ficheiro carregado com sucesso!</span>';
      setTimeout(() => {
        if (statusDiv) statusDiv.innerHTML = "";
      }, 3000);
    }
  ...
  // Modificar onProcess para mostrar loading e sucesso
  attachGlobalEvents() {
    const processBtn = this.element.querySelector("#processBrokersBtn");
    if (processBtn) {
      processBtn.addEventListener("click", async () => {
        if (this.onProcessCallback) {
          ...
          try {
            ...
            // Restaurar botão (será recarregado pela navegação)
            btn.disabled = false;
            btnText.style.display = "inline";
            btnLoader.style.display = "none";
            btn.classList.remove("processing");
