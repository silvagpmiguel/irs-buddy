class IRSBuddy {
  ...
  displayResults(analysis) {
    const resultHTML = `
            <div class="analysis-stats">
                ...
                <div class="stat-card">
                    <span class="stat-value">€ ${Math.abs(analysis.taxInfo.estimatedRefund).toLocaleString()}</span>
                    <span class="stat-label">${analysis.taxInfo.estimatedRefund >= 0 ? "Reembolso Estimado" : "Valor a Pagar"}</span>
                </div>
            </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    <tr><td>Despesas de Saúde</td><td>€ ${analysis.deductions.health.toFixed(2)}</td></tr>
                    <tr><td>Despesas de Educação</td><td>€ ${analysis.deductions.education.toFixed(2)}</td></tr>
                    <tr><td>Despesas de Habitação</td><td>€ ${analysis.deductions.housing.toFixed(2)}</td></tr>
                    <tr><td>Deduções Gerais</td><td>€ ${analysis.deductions.general.toFixed(2)}</td></tr>
                </tbody>
            </table>
            ...
            <div style="background: #fff3e0; padding: 1rem; border-radius: 8px; margin-bottom: 1.5rem;">
                ${analysis.warnings.map((w) => `<div style="margin-bottom: 0.5rem;">${w}</div>`).join("")}
            </div>
            
            <h3 style="margin-bottom: 1rem; color: var(--primary-dark);">💡 Recomendações</h3>
            <ul style="margin-left: 1.5rem; color: var(--text-gray);">
                ${analysis.recommendations.map((r) => `<li style="margin-bottom: 0.5rem;">${r}</li>`).join("")}
            </ul>
        `;

    this.resultContent.innerHTML = resultHTML;
    this.resultSummary.textContent = `Análise concluída para ${analysis.fileName}`;
  ...
  downloadReport() {
    ...
    a.href = url;
    a.download = `irs_buddy_report_${new Date().toISOString().slice(0, 19)}.txt`;
...
${analysis.taxInfo.estimatedRefund >= 0 ? "Reembolso Estimado" : "Valor a Pagar"}: € ${Math.abs(analysis.taxInfo.estimatedRefund).toLocaleString()}
